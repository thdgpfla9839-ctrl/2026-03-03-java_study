...
//        	FoodDAO dao=FoodDAO.newInstance();
        	...
        	for(int i=1;i<=346;i++)
        	{
        		...
        		for(int j=0;j<link.size();j++)
        		{
        		  ...
        		  // 파일에 저장
        		   FileWriter fw=new FileWriter("c:\\javaDev\\food.txt",true); // true 혹은 append를 줘야 바로 밑에 연결되서 붙어서 출력된다
        		                                                               // w -> writer : create(덮어쓰기)
        		                                                               // r -> read : read
        		                                                               // a-> append : 쓴거 밑에 연결돼서 출력
        		   
        		   String data=m+"|"+vo.getName()+"|"+vo.getType()+"|"
        		                    +vo.getPhone()+"|"+vo.getAddress()+"|"
        				            +vo.getScore()+"|"+vo.getParking()+"|"
        		                    +vo.getPoster()+"|"+vo.getTime()+"|"
        		                    +vo.getContent()+"|"+vo.getTheme()+"|"
        		                    +vo.getPrice()+"\r\n";
